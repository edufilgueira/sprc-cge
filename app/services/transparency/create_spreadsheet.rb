class Transparency::CreateSpreadsheet
  ...
  def presenter_klass
    "Transparency::Export::#{transparency_export.camelized_resource}Presenter".constantize
  end
  ...
  def add_rows_to_file(sheet)
    # Algumas consultas são aprensentadas com a estrutura de árvore para fazer o consolidados de valores
    # Desta forma o presenter retorna o array de linhas para montar a planilha
    # Ex: Receitas do poder executivo, Despesas do poder executivo
    if presenter_klass.tree_structure
      rows(filtered_resources).each do |row|
  ...
  def filename
    #
    # XXX Se um arquivo com a mesma query já está sendo processado,
    # o serviço aguarda em um loop o processamento até o dia seguinte
    # para ultilizar o mesmo arquivo e economizar processamento
    #
    return build_filename unless transparency_already_created.present?

    return transparency_already_created.filename if transparency_already_created.success?

    filename_already_created = ''

    delay = 60 #seconds
    end_time = Time.now.tomorrow.beginning_of_day
    #
    # XXX
    # XXX Descomentar aqui para testes do loop
    # XXX
    #
    # delay = 1 # to test
    # end_time = Time.now + 3.seconds # to test
    #
    #
    loop do
      filename_already_created = transparency_already_created.filename

      break if transparency_already_created.success? || Time.now > end_time

      log(:info, "[TransparencyExport] Export ##{transparency_export.id} waiting for export ##{transparency_already_created.id}")

      sleep delay
    end

    filename_already_created.present? ? filename_already_created : build_filename
  end
  ...
  def transparency_already_created
    ...
    #
    Transparency::Export.where.not('id >= ?', @transparency_export).find_by(params)
  end
