...
describe Reports::Tickets::ServiceTypePresenter do
  ...
  subject(:presenter) { Reports::Tickets::ServiceTypePresenter.new(scope, ticket_report) }

  describe 'self.call' do
    it 'initialize' do
      allow(Reports::Tickets::ServiceTypePresenter).to receive(:new)

      presenter

      expect(Reports::Tickets::ServiceTypePresenter).to have_received(:new).with(scope, ticket_report)
    end
  end
  ...
  describe 'helpers' do
    context 'rows' do
      context 'when cge operator' do
        ...
        let(:service_type_1) { create(:service_type, name: 'service_type_1') }
        let(:service_type_2) { create(:service_type, name: 'service_type_2') }
        ...
        before do
          ...
          create(:classification, ticket: ticket, service_type: service_type_1)

          ticket = create(:ticket, :with_parent, organ: organ_a)
          create(:classification, ticket: ticket, service_type: service_type_2)
      ...
      context 'when sectoral operator' do
        ...
        before do
          ...
          reopened = create(:ticket, :with_parent, :with_reopen_and_log, reopened_count: 2, organ: organ_a)
          create(:classification, ticket: reopened, service_type: service_type_1)
        end
