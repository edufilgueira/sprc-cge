...
describe Reports::Tickets::Solvability::SicService do
  ...
  let(:solvability_report) { create(:solvability_report, filters: { ticket_type: :sic, confirmed_at: { start: beginning_date, end: end_date} }) }
  ...
  subject(:service) { Reports::Tickets::Solvability::SicService.new(xls_workbook, solvability_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Solvability::SicService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Solvability::SicService.call(xls_workbook, solvability_report)

      expect(Reports::Tickets::Solvability::SicService).to have_received(:new).with(xls_workbook, solvability_report)
  ...
  describe 'call service' do
    context 'invokes presenter with scope' do
      ...
      let(:scope) do
        scope = solvability_report.filtered_scope
        scope = scope.without_other_organs.without_internal_status(Ticket::INVALIDATED_STATUSES)
        scope = scope.left_joins(parent: :attendance)

        # csai_only
        scope.where(attendances: { id: nil }).or(
          scope.where(attendances: { service_type: [:sic_forward, :sou_forward] }))
      end
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.solvability.sic.sheet_name")
