...
describe Reports::Tickets::Report::Sou::InternalStatusService do
  ...
  let(:presenter) { Reports::Tickets::Sou::InternalStatusPresenter.new(default_scope, ticket_report) }
  subject(:service) { Reports::Tickets::Report::Sou::InternalStatusService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::InternalStatusService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::InternalStatusService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::InternalStatusService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::Sou::InternalStatusPresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::Sou::InternalStatusPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.internal_status.sheet_name")
      ...
      it 'header' do
        headers = Reports::Tickets::Sou::InternalStatusPresenter::COLUMNS.map do |column|
          I18n.t("services.reports.tickets.sou.internal_status.headers.#{column}")
        end
