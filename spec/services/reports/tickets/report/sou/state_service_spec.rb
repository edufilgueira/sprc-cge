...
describe Reports::Tickets::Report::Sou::StateService do
  ...
  let(:presenter) { Reports::Tickets::StatePresenter.new(default_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sou::StateService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::StateService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::StateService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::StateService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.state.sheet_name")
