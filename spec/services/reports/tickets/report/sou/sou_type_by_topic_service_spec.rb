...
describe Reports::Tickets::Report::Sou::SouTypeByTopicService do
  ...
  let(:presenter) { Reports::Tickets::Sou::SouTypeByTopicPresenter.new(default_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sou::SouTypeByTopicService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::SouTypeByTopicService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::SouTypeByTopicService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::SouTypeByTopicService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::Sou::SouTypeByTopicPresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::Sou::SouTypeByTopicPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.sou_type_by_topic.sheet_name")
