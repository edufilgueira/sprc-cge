...
describe Reports::Tickets::Report::Sou::ServiceTypeService do
  ...
  let(:presenter) { Reports::Tickets::ServiceTypePresenter.new(default_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sou::ServiceTypeService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::ServiceTypeService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::ServiceTypeService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::ServiceTypeService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.service_type.sheet_name")
        ...
        context 'when cge operator' do
          it 'include organ' do
            headers = Reports::Tickets::ServiceTypePresenter::COLUMNS.map do |column|
              I18n.t("services.reports.tickets.sou.service_type.headers.#{column}")
            end
