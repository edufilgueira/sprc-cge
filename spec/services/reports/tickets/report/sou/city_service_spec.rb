...
describe Reports::Tickets::Report::Sou::CityService do
  ...
  let(:presenter) { Reports::Tickets::CityPresenter.new(default_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sou::CityService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::CityService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::CityService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::CityService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::CityPresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::CityPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.city.sheet_name")
      ...
      it 'header' do
        headers = Reports::Tickets::CityPresenter::COLUMNS.map do |column|
          I18n.t("services.reports.tickets.sou.city.headers.#{column}")
        end
      ...
      it 'rows' do
        ...
        expect(service).to receive(:xls_add_row).with(anything, [I18n.t("services.reports.tickets.sic.city.total"), presenter.total_count])
