...
describe Reports::Tickets::Report::Sou::TopicService do
  ...
  let(:presenter) { Reports::Tickets::TopicPresenter.new(default_scope, ticket_report) }
  subject(:service) { Reports::Tickets::Report::Sou::TopicService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::TopicService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::TopicService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::TopicService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::TopicPresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::TopicPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.topic.sheet_name")
      ...
      context 'header' do
        context 'when cge operator' do
          it 'include organ' do
            topic_title = I18n.t("services.reports.tickets.sou.topic.title")
            organ_title = I18n.t("services.reports.tickets.sou.topic.organ.title")
            count_title = I18n.t("services.reports.tickets.sou.topic.count.title")
            percentage_title = I18n.t("services.reports.tickets.sou.topic.percentage.title")

            allow(service).to receive(:xls_add_header)
            expect(service).to receive(:xls_add_header).with(anything, [ topic_title, organ_title, count_title, percentage_title ])
        ...
        context 'when sectoral operator' do
          ...
          it 'exclude organ' do
            ...
            allow(service).to receive(:xls_add_header)
            expect(service).to receive(:xls_add_header).with(anything, [ topic_title, count_title, percentage_title ])
