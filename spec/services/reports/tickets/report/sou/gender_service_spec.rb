...
describe Reports::Tickets::Report::Sou::GenderService do
  ...
  let(:presenter) { Reports::Tickets::Sou::GenderPresenter.new(default_scope, ticket_report) }
  subject(:service) { Reports::Tickets::Report::Sou::GenderService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::GenderService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::GenderService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::GenderService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::Sou::GenderPresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::Sou::GenderPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.gender.sheet_name")
      ...
      it 'header' do
        headers = Reports::Tickets::Sou::GenderPresenter::COLUMNS.map do |column|
