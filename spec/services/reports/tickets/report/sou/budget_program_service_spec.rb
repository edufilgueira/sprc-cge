...
describe Reports::Tickets::Report::Sou::BudgetProgramService do
  ...
  let(:presenter) { Reports::Tickets::BudgetProgramPresenter.new(default_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sou::BudgetProgramService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::BudgetProgramService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::BudgetProgramService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::BudgetProgramService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      it { expect(Reports::Tickets::BudgetProgramPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.budget_program.sheet_name")
