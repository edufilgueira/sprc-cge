...
describe Reports::Tickets::Report::Sou::AverageAnswerDepartmentService do
  ...
  let(:presenter) { Reports::Tickets::AverageAnswerDepartmentPresenter.new(default_scope) }
  subject(:service) { Reports::Tickets::Report::Sou::AverageAnswerDepartmentService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sou::AverageAnswerDepartmentService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sou::AverageAnswerDepartmentService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sou::AverageAnswerDepartmentService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sou.average_answer_department.sheet_name")
      ...
      it 'header' do
        headers = Reports::Tickets::AverageAnswerDepartmentPresenter::COLUMNS.map do |column|
          I18n.t("services.reports.tickets.sou.average_answer_department.headers.#{column}")
        end
