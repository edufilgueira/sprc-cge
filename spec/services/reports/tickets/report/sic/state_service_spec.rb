...
describe Reports::Tickets::Report::Sic::StateService do
  ...
  let(:presenter) { Reports::Tickets::StatePresenter.new(report_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sic::StateService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sic::StateService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sic::StateService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sic::StateService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      before do
        ...
        allow(Reports::Tickets::StatePresenter).to receive(:new).and_call_original

        service.call
      end


      it { expect(Reports::Tickets::StatePresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sic.state.sheet_name")
      ...
      it 'header' do
        headers = Reports::Tickets::StatePresenter::COLUMNS.map do |column|
          I18n.t("services.reports.tickets.sic.state.headers.#{column}")
        end
      ...
      it 'rows' do
        ...
        expect(service).to receive(:xls_add_row).with(anything, [I18n.t("services.reports.tickets.sic.state.total"), presenter.total_count])
