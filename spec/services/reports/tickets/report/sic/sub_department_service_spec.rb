...
describe Reports::Tickets::Report::Sic::SubDepartmentService do
  ...
  let(:presenter) { Reports::Tickets::SubDepartmentPresenter.new(report_scope, ticket_report) }

  subject(:service) { Reports::Tickets::Report::Sic::SubDepartmentService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sic::SubDepartmentService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sic::SubDepartmentService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sic::SubDepartmentService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      let(:ticket_department_1) do
        ticket = create(:ticket, :sic, :with_parent_sic)
        create(:classification, department: department_1, sub_department: department_1_sub_department_1, ticket: ticket)
        ticket
      end

      let(:ticket_department_2) do
        ticket = create(:ticket, :sic, :with_parent_sic)
        create(:classification, department: department_2, sub_department: department_2_sub_department_2, ticket: ticket)
        ticket
      end


      let(:scope) do
        report_scope.without_other_organs.without_internal_status(Ticket::INVALIDATED_STATUSES)
      end
      ...
      it { expect(Reports::Tickets::SubDepartmentPresenter).to have_received(:new).with(match_array(scope), ticket_report) }
    end

    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sic.sub_department.sheet_name")
