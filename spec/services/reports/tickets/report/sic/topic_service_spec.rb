...
describe Reports::Tickets::Report::Sic::TopicService do
  ...
  let(:presenter) { Reports::Tickets::TopicPresenter.new(report_scope, ticket_report) }
  subject(:service) { Reports::Tickets::Report::Sic::TopicService.new(xls_workbook, ticket_report) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Tickets::Report::Sic::TopicService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Tickets::Report::Sic::TopicService.call(xls_workbook, ticket_report)

      expect(Reports::Tickets::Report::Sic::TopicService).to have_received(:new).with(xls_workbook, ticket_report)
  ...
  describe 'call service' do
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.tickets.sic.topic.sheet_name")
      ...
      context 'header' do
        context 'when cge operator' do
          it 'include organ' do
            topic_title = I18n.t("services.reports.tickets.sic.topic.title")
            organ_title = I18n.t("services.reports.tickets.sic.topic.organ.title")
            count_title = I18n.t("services.reports.tickets.sic.topic.count.title")
            percentage_title = I18n.t("services.reports.tickets.sic.topic.percentage.title")
