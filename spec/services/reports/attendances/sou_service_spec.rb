...
describe Reports::Attendances::SouService do
  ...
  subject(:service) { Reports::Attendances::SouService.new(xls_workbook, beginning_date, end_date) }

  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Reports::Attendances::SouService).to receive(:new) { service }
      allow(service).to receive(:call)

      Reports::Attendances::SouService.call(xls_workbook, beginning_date, end_date)

      expect(Reports::Attendances::SouService).to have_received(:new).with(xls_workbook, beginning_date, end_date)
  ...
  describe 'call service' do
    ...
    context 'invokes presenter with scope' do
      ...
      let(:scope) { [attendance_sou_forward] }
    ...
    context 'worksheet' do
      it 'sheet_name' do
        expected = I18n.t("services.reports.attendances.sou.sheet_name").slice(0, 25)
      ...
      context 'headers' do
        it 'add_attendance_header' do
          title = [ I18n.t("services.reports.attendances.sou.attendance_title") ]
        ...
        it 'add_returned_phone_header' do
          title = [
            I18n.t("services.reports.attendances.sou.returned_phone_title"),
            I18n.t("services.reports.attendances.sou.returned_phone_header_with_success_title"),
            I18n.t("services.reports.attendances.sou.returned_phone_header_without_success_title"),
            I18n.t("services.reports.attendances.sou.returned_phone_header_attemps_title"),
            I18n.t("services.reports.attendances.sou.returned_phone_header_total_protocols_title")
          ]
        ...
        it 'add_returned_whatsapp_header' do
          title = [
            I18n.t("services.reports.attendances.sou.returned_whatsapp_title"),
            I18n.t("services.reports.attendances.sou.returned_whatsapp_header_total_returned_title")
          ]
      ...
      context 'rows' do
        ...
        before do
          create(:attendance, :sou_forward, created_by_id: call_center_1.id)
          create(:attendance, :sou_forward, created_by_id: call_center_1.id)
          create(:attendance, :sou_forward, created_by_id: call_center_2.id)
          create(:attendance, :sou_forward, created_by_id: call_center_supervisor.id)

          ticket_phone_return = create(:ticket, answer_type: :phone, answer_phone: 'teste 123', ticket_type: :sou)
          ticket_whatsapp_return = create(:ticket, answer_type: :whatsapp, answer_whatsapp: 'teste 123', ticket_type: :sou)
        ...
        context 'attendances_by_user' do
          it { expect(service).to have_received(:xls_add_row).with(anything, [ call_center_1.name, 2 ]) }
          it { expect(service).to have_received(:xls_add_row).with(anything, [ call_center_2.name, 1 ]) }
