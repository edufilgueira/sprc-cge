...
describe Ticket::Solvability::SectoralService do

  let(:solvability) { Ticket::Solvability::SectoralService.new(scope, organ, start_date, end_date) }
  ...
  let(:confirmed_reopened) do
    ...
    t.answers.first.update(sectoral_deadline: -3)
    t
  end
  ...
  describe 'self.call' do
    it 'initialize and invoke call method' do
      service = double
      allow(Ticket::Solvability::SectoralService).to receive(:new) { service }
      allow(service).to receive(:call)
      Ticket::Solvability::SectoralService.call(scope, organ, start_date, end_date)

      expect(Ticket::Solvability::SectoralService).to have_received(:new).with(scope, organ, start_date, end_date)
