...
describe PPA::SourceMappers::Biennial::RegionalProductSourceMapper do
  ...
  describe '::map' do
    ...
    context 'with all pre-requisites satisfied' do
      ...
      context 'when the target biennial regional product do not exist' do

        it 'creates the two related regional product' do
          expect { map }.to change { product.biennial_regional_products.count }.by(1)

          target = product.biennial_regional_products.last
      ...
      context 'when the target biennial regional product already exists' do
        let!(:target) do
        ...
        it 'does not create any biennial regional products' do
