...
describe PPA::SourceMappers::Annual::RegionalInitiativeSourceMapper do
  ...
  describe '::map' do
    ...
    context 'with all pre-requisites satisfied' do
      ...
      context 'when the target annual regional initiatives do not exist' do

        it 'creates the two related regional initiatives (one for each biennium year)' do
          expect { map }.to change { PPA::Annual::RegionalInitiative.count }.by(2)

          first_year_regional_initiative  = initiative.annual_regional_initiatives
            .in_year_and_region(source.biennium.first_year, region)
            .last
          second_year_regional_initiative = initiative.annual_regional_initiatives
      ...
      context 'when the target annual regional initiatives already exists' do
        let!(:first_year_regional_initiative) do
          create :ppa_annual_regional_initiative,
                 initiative: initiative,
                 year: source.biennium.first,
                 region: region
        end

        let!(:second_year_regional_initiative) do
          create :ppa_annual_regional_initiative,
                 initiative: initiative,
        ...
        it 'does not create any annual regional initiatives' do
