...
describe Integration::Expenses::ExpensesTreeNodes::Function do
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::Function.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::Function.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(function)
      expect(node[:id]).to eq(function.codigo_funcao)
      expect(node[:type]).to eq(:function)
      expect(node[:title]).to eq(function.title)
