...
describe Integration::Expenses::ExpensesTreeNodes::AdministrativeRegion do
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::AdministrativeRegion.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::AdministrativeRegion.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(administrative_region)
      expect(node[:id]).to eq(administrative_region.codigo_regiao_resumido)
      expect(node[:type]).to eq(:administrative_region)
      expect(node[:title]).to eq(administrative_region.title)
