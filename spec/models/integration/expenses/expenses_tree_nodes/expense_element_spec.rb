...
describe Integration::Expenses::ExpensesTreeNodes::ExpenseElement do
  ...
  let(:expense_element) { create(:integration_supports_expense_element, codigo_elemento_despesa: '56') }
  ...
  let(:valor_inicial_attributes) do
    {
      ...
      cod_natureza_desp: "1234#{expense_element.codigo_elemento_despesa}789",
      cod_elemento_despesa: expense_element.codigo_elemento_despesa,
      valor_inicial: 100,
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::ExpenseElement.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::ExpenseElement.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(expense_element)
      expect(node[:id]).to eq(expense_element.codigo_elemento_despesa)
      expect(node[:type]).to eq(:expense_element)
      expect(node[:title]).to eq(expense_element.title)
