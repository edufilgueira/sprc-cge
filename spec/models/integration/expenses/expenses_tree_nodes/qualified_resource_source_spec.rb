...
describe Integration::Expenses::ExpensesTreeNodes::QualifiedResourceSource do
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::QualifiedResourceSource.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::QualifiedResourceSource.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(qualified_resource_source)
      expect(node[:id]).to eq(qualified_resource_source.codigo)
      expect(node[:type]).to eq(:qualified_resource_source)
      expect(node[:title]).to eq(qualified_resource_source.title)
