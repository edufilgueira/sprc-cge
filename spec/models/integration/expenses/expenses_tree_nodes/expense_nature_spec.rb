...
describe Integration::Expenses::ExpensesTreeNodes::ExpenseNature do
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::ExpenseNature.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::ExpenseNature.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(expense_nature)
      expect(node[:id]).to eq(expense_nature.codigo_natureza_despesa)
      expect(node[:type]).to eq(:expense_nature)
      expect(node[:title]).to eq(expense_nature.title)
