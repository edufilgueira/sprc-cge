...
describe Integration::Expenses::ExpensesTreeNodes::GovernmentProgram do
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::GovernmentProgram.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::GovernmentProgram.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(government_program)
      expect(node[:id]).to eq(government_program.id)
      expect(node[:type]).to eq(:government_program)
      expect(node[:title]).to eq(government_program.title)
