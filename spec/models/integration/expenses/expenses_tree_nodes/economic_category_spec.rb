...
describe Integration::Expenses::ExpensesTreeNodes::EconomicCategory do
  ...
  let(:economic_category) { create(:integration_supports_economic_category, codigo_categoria_economica: '1') }
  ...
  let(:valor_inicial_attributes) do
    {
      ...
      cod_natureza_desp: "#{economic_category.codigo_categoria_economica}23456789",
      valor_inicial: 100,
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::EconomicCategory.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::EconomicCategory.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(economic_category)
      expect(node[:id]).to eq(economic_category.codigo_categoria_economica)
      expect(node[:type]).to eq(:economic_category)
      expect(node[:title]).to eq(economic_category.title)
