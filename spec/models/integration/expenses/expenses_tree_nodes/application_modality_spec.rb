...
describe Integration::Expenses::ExpensesTreeNodes::ApplicationModality do
  ...
  let(:application_modality) { create(:integration_supports_application_modality, codigo_modalidade: '34') }
  ...
  let(:valor_inicial_attributes) do
    {
      ...
      cod_natureza_desp: "12#{application_modality.codigo_modalidade}56789",
      valor_inicial: 100,
  ...
  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Expenses::ExpensesTreeNodes::ApplicationModality.new(Integration::Expenses::BudgetBalance.none)
  ...
  describe 'calculations' do
    it 'returns nodes' do
      ...
      nodes = Integration::Expenses::ExpensesTreeNodes::ApplicationModality.new(Integration::Expenses::BudgetBalance.all).nodes

      node = nodes.first

      expect(node[:resource]).to eq(application_modality)
      expect(node[:id]).to eq(application_modality.codigo_modalidade)
      expect(node[:type]).to eq(:application_modality)
      expect(node[:title]).to eq(application_modality.title)
