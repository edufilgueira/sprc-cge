...
describe Integration::Revenues::RevenuesTreeNodes::CategoriaEconomica do


  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Revenues::RevenuesTreeNodes::CategoriaEconomica.new(Integration::Revenues::Account.none)
  ...
  describe 'calculations' do
    ...
    describe 'valor_previsto_inicial' do
      it 'returns nodes' do
        ...
        nodes = Integration::Revenues::RevenuesTreeNodes::CategoriaEconomica.new(Integration::Revenues::Account.all).nodes

        expect(nodes.count).to eq(2)

        first_node = nodes.first

        expect(first_node[:id]).to eq(first_revenue_nature_categoria_economica.unique_id_categoria_economica)
        expect(first_node[:type]).to eq(:categoria_economica)
        expect(first_node[:title]).to eq(first_revenue_nature_categoria_economica.title)
        expect(first_node[:valor_previsto_inicial]).to eq(first_valor_previsto_inicial)

        second_node = nodes.second

        expect(second_node[:id]).to eq(second_revenue_nature_categoria_economica.unique_id_categoria_economica)
        expect(second_node[:type]).to eq(:categoria_economica)
        expect(second_node[:title]).to eq(second_revenue_nature_categoria_economica.title)
