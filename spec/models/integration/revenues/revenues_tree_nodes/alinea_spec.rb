...
describe Integration::Revenues::RevenuesTreeNodes::Alinea do


  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Revenues::RevenuesTreeNodes::Alinea.new(Integration::Revenues::Account.none)
  ...
  describe 'calculations' do
    ...
    let(:first_revenue_nature_alinea) { create(:integration_supports_revenue_nature, codigo: '112109900')}

    let(:second_revenue_nature_subalinea) { create(:integration_supports_revenue_nature, codigo: '813250199')}
    let(:second_revenue_nature_alinea) { create(:integration_supports_revenue_nature, codigo: '813250100')}
    ...
    describe 'valor_previsto_inicial' do
      it 'returns nodes' do
        ...
        first_revenue_nature_alinea
        second_revenue_nature_alinea
        ...
        nodes = Integration::Revenues::RevenuesTreeNodes::Alinea.new(Integration::Revenues::Account.all).nodes

        expect(nodes.count).to eq(2)

        first_node = nodes.first

        expect(first_node[:type]).to eq(:alinea)
        expect(first_node[:title]).to eq(first_revenue_nature_alinea.title)
        expect(first_node[:valor_previsto_inicial]).to eq(first_valor_previsto_inicial)

        second_node = nodes.second

        expect(second_node[:type]).to eq(:alinea)
        expect(second_node[:title]).to eq(second_revenue_nature_alinea.title)
