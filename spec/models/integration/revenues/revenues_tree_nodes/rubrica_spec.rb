...
describe Integration::Revenues::RevenuesTreeNodes::Rubrica do


  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Revenues::RevenuesTreeNodes::Rubrica.new(Integration::Revenues::Account.none)
  ...
  describe 'calculations' do
    ...
    let(:first_revenue_nature_rubrica) { create(:integration_supports_revenue_nature, codigo: '112109900')}

    let(:second_revenue_nature_subalinea) { create(:integration_supports_revenue_nature, codigo: '813250199')}
    let(:second_revenue_nature_rubrica) { create(:integration_supports_revenue_nature, codigo: '813250000')}
    ...
    describe 'valor_previsto_inicial' do
      it 'returns nodes' do
        ...
        first_revenue_nature_rubrica
        second_revenue_nature_rubrica

        previsao_inicial_revenue

        expect(first_revenue_nature_subalinea.codigo_rubrica).to eq(first_revenue_nature_rubrica.codigo)
        expect(second_revenue_nature_subalinea.codigo_rubrica).to eq(second_revenue_nature_rubrica.codigo)
        ...
        nodes = Integration::Revenues::RevenuesTreeNodes::Rubrica.new(Integration::Revenues::Account.all).nodes

        expect(nodes.count).to eq(2)

        first_node = nodes.first

        expect(first_node[:type]).to eq(:rubrica)
        expect(first_node[:title]).to eq(first_revenue_nature_rubrica.title)
        expect(first_node[:valor_previsto_inicial]).to eq(first_valor_previsto_inicial)

        second_node = nodes.second

        expect(second_node[:type]).to eq(:rubrica)
        expect(second_node[:title]).to eq(second_revenue_nature_rubrica.title)
