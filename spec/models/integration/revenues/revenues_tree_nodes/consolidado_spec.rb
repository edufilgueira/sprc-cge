...
describe Integration::Revenues::RevenuesTreeNodes::Consolidado do


  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Revenues::RevenuesTreeNodes::Consolidado.new(Integration::Revenues::Account.none)
  ...
  describe 'calculations' do
    ...
    describe 'valor_previsto_inicial' do
      it 'returns nodes' do
        ...
        nodes = Integration::Revenues::RevenuesTreeNodes::Consolidado.new(Integration::Revenues::Account.all).nodes

        expect(nodes.count).to eq(2)

        first_node = nodes.first

        expect(first_node[:id]).to eq(first_revenue_nature_consolidado.unique_id_consolidado)
        expect(first_node[:type]).to eq(:consolidado)
        expect(first_node[:title]).to eq(first_revenue_nature_consolidado.title)
        expect(first_node[:valor_previsto_inicial]).to eq(first_valor_previsto_inicial)

        second_node = nodes.second

        expect(second_node[:id]).to eq(second_revenue_nature_consolidado.unique_id_consolidado)
        expect(second_node[:type]).to eq(:consolidado)
        expect(second_node[:title]).to eq(second_revenue_nature_consolidado.title)
