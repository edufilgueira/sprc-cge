...
describe Integration::Revenues::TransfersTreeNodes::Transfer do

  describe 'initialization' do
    it 'returns empty array for empty scope' do
      nodes = Integration::Revenues::TransfersTreeNodes::Transfer.new(Integration::Revenues::Transfer.none)
  ...
  describe 'calculations' do
    ...
    describe 'valor_previsto_inicial' do
      it 'returns nodes' do
        ...
        nodes = Integration::Revenues::TransfersTreeNodes::Transfer.new(Integration::Revenues::Transfer.all).nodes

        expect(nodes.count).to eq(2)

        first_node = nodes.first

        expect(first_node[:resource]).to eq(:required)
        expect(first_node[:id]).to eq(:required)
        expect(first_node[:type]).to eq(:transfer)
        expect(first_node[:title]).to eq(Integration::Supports::RevenueNature.human_attribute_name('transfer_required'))
        expect(first_node[:valor_previsto_inicial]).to eq(first_valor_previsto_inicial)

        second_node = nodes.second

        expect(second_node[:resource]).to eq(:voluntary)
        expect(second_node[:id]).to eq(:voluntary)
        expect(second_node[:type]).to eq(:transfer)
        expect(first_node[:title]).to eq(Integration::Supports::RevenueNature.human_attribute_name('transfer_required'))
