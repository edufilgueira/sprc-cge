...
describe Abilities::Users::Operator::Chief do

  let(:organ) { create(:executive_organ) }
  let(:other_organ) { create(:executive_organ) }
  let(:user) { create(:user, :operator_chief, organ: organ) }

  subject(:ability) { Abilities::Users::Operator::Chief.new(user) }
  ...
  describe 'Ticket' do
    ...
    describe '#second_extend' do
      describe '#can_extend_ticket_second_time' do
        let(:ticket) { create(:ticket, :with_parent, :in_sectoral_attendance, organ: organ) }
        let(:ticket_with_extension) { create(:ticket, :with_extension, organ: organ) }

        let(:ticket_out_limit_extend) do
          ticket_days = Ticket::LIMIT_TO_EXTEND_DEADLINE + 5
          exceeded_limit = DateTime.now - ticket_days.days
          create(:ticket, :with_extension, deadline: Ticket::SOU_DEADLINE - ticket_days, confirmed_at: exceeded_limit, organ: organ)
        end

        context 'when ticket is in time to have a second extend' do
          it 'be able to second extend' do
            is_expected.to be_able_to(:second_extend, ticket_with_extension)
          end
        end

        context 'when ticket is out of limit to a second extend' do
          it 'not be able to second extend' do
            is_expected.not_to be_able_to(:second_extend, ticket_out_limit_extend)
          end
        end

        context 'when another extension created with a first solicitation' do
          context 'in_progress' do
            it 'not be able to second extend' do
              create(:extension, ticket: ticket, status: :in_progress)
              is_expected.not_to be_able_to(:second_extend, ticket)
            end
          end
          ...
          context 'approved' do
            it 'be able to second extend' do
              ...
              ticket.reload
              is_expected.to be_able_to(:second_extend, ticket)
            end
          end
        ...
        context 'when another extension created with a second solicitation' do
          context 'in_progress' do
            it 'not be able to second extend' do
              create(:extension, :second, ticket: ticket_with_extension, status: :in_progress)
              ticket_with_extension.reload
              is_expected.not_to be_able_to(:second_extend, ticket_with_extension)
            end
          end

          context 'approved' do
            it 'not be able to second extend' do
              create(:extension, :second, ticket: ticket_with_extension, status: :approved)
              ticket_with_extension.reload
              is_expected.not_to be_able_to(:second_extend, ticket_with_extension)
            end
          end
          ...
          context 'rejected' do
            it 'be able to second extend' do
              create(:extension, :second, ticket: ticket_with_extension, status: :rejected)
              ticket_with_extension.reload
              is_expected.to be_able_to(:second_extend, ticket_with_extension)
            end
          end
          ...
          context 'cancelled' do
            it 'be able to second extend' do
              create(:extension, :second, ticket: ticket_with_extension, status: :cancelled)
              ticket_with_extension.reload
              is_expected.to be_able_to(:second_extend, ticket_with_extension)
            end
          end
        ...
        context 'when ticket is sic' do
          let(:ticket_with_extension) { create(:ticket, :with_extension, :sic, organ: organ) }
        ...
        context 'when ticket have a final answer' do
          let(:ticket_replied) { create(:ticket, :with_extension, :replied, organ: organ) }

          it 'not be able to second extend' do
            is_expected.not_to be_able_to(:second_extend, ticket_replied)
          end
        end

        context 'when ticket expired' do
          let(:ticket_with_extension) { create(:ticket, :with_extension, :expired_can_extend, organ: organ) }
    ...
    describe '#cancel_second_extend' do
      describe '#can_cancel_second_extend' do
        let(:ticket) { create(:ticket, :with_parent, :in_sectoral_attendance, organ: organ) }
        let(:ticket_with_extension) { create(:ticket, :with_extension, organ: organ) }
        ...
        context 'when ticket is in time to have a second extend' do
          it 'not be able to cancel second extend' do
            is_expected.not_to be_able_to(:cancel_second_extend, ticket_with_extension)
          end
        end

        context 'when ticket is out of limit to a second extend' do
          it 'not be able to second extend' do
            is_expected.not_to be_able_to(:cancel_second_extend, ticket_out_limit_extend)
          end
        end
        ...
        context 'when another extension created with a first solicitation' do
          context 'in_progress' do
            it 'not be able to second extend' do
              create(:extension, ticket: ticket, status: :in_progress)
              is_expected.not_to be_able_to(:cancel_second_extend, ticket)
            end
          end
        ...
        context 'when another extension created with a second solicitation' do
          context 'in_progress' do
            it 'not be able to second extend' do
              create(:extension, :second, ticket: ticket_with_extension, status: :in_progress)
              ticket_with_extension.reload
              is_expected.to be_able_to(:cancel_second_extend, ticket_with_extension)
            end
          end
        ...
        context 'when ticket have a final answer' do
          ...
          it 'not be able to second extend' do
            is_expected.not_to be_able_to(:cancel_second_extend, ticket_replied)
          end
        end
    ...
    context 'extension' do
      ...
      context 'with ticket' do
        context 'from other organ' do
          let(:ticket_other_organ) { create(:ticket, :with_parent, :in_sectoral_attendance, organ: other_organ) }

          it { is_expected.not_to be_able_to(:approve_extension, ticket_other_organ) }
        end

        context 'with ticket from the same organ' do
      ...
      context 'when ticket replied' do
        ...
        it { is_expected.not_to be_able_to(:approve_extension, ticket_replied) }
        it { is_expected.not_to be_able_to(:reject_extension, ticket_replied) }
      end
    end
    ...
    context 'forward' do
      context 'classified' do
        before do
          ...
          create(:classification, ticket: ticket_from_other_organ)
        end
    ...
    context 'reopen' do
      ...
      let(:ticket_sic_created_by_user) { create(:ticket, :sic, :replied, created_by: user) }
      ...
      it { is_expected.to be_able_to(:reopen, ticket_sic_created_by_user) }
    ...
    context 'organ_association' do
      it { is_expected.to be_able_to(:view_organ_association, User) }
    end
  end
