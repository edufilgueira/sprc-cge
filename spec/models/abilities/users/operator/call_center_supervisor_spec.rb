...
describe Abilities::Users::Operator::CallCenterSupervisor do

  let(:user) { create(:user, :operator_call_center) }

  subject(:ability) { Abilities::Users::Operator::CallCenterSupervisor.new(user) }
  ...
  describe 'AttendanceReport' do
    it { is_expected.to be_able_to(:create, AttendanceReport) }
  end
  ...
  describe 'Answer type option' do
    let(:user_operator_call_center_supervisor) { create(:user, :operator_call_center_supervisor) }

    context 'answer option with letter' do
      it {
        is_expected.to be_able_to(:answer_by_letter, user_operator_call_center_supervisor)
      }
    end
    context 'answer option with phone' do
      it {
        is_expected.to be_able_to(:answer_by_phone, user)
      }
    end
  end
